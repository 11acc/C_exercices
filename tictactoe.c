...
//
// 2-player tic tac toe game in command line
...
int main() {
    ...
    int xScore = gameState.p1Score;
    int oScore = gameState.p2Score;
    int counter = gameState.currPlayer; // counter 0 = X

    char boardVals[9]; // should be empty '         ' if new game
    for (int i = 0; i < 9; i++) {
        boardVals[i] = gameState.board[i];
    }
    
    // other vars for code functionality
    int gameContinues = 1, win=0;
    char player = 'X';

    // main game loop - every move made this repeats
    do {
        printf("\nScore: %d | %d\n", xScore, oScore);

        // make board
        displayGrid(boardVals);

        // draw check - check if there are any spaces left to play
        isDraw(&gameContinues, boardVals);
        if (!gameContinues) {
        ...
        // select player
        player = checkPlayerOrder(counter);
        printf("Player: %c\n", player);

        // make a move
        printf("Choose a space in the board (0-8 couting left to right)\n");
        int move;
        isValidMove(&move, boardVals);
        // make the move
        boardVals[move] = player;

        // win check - check if current player wins
        win = checkWin(boardVals, player);
        if (win) {
            // display winning board
            displayGrid(boardVals);
            gameContinues = 0; // game ends
        ...
        // update game state and save it
        updateNSave(&gameState, xScore, oScore, counter, win, boardVals);
